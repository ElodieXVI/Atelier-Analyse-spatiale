...
library(ggplot2)
library(patchwork)
setwd("C:/Users/clemm/Desktop/MASTER QESS 2ème année/Cours M2/Analyse spatiale/Atelier-Analyse-spatiale")
...
base_class_ens <- read_delim("classification_ens.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
classens <- base_class_ens %>% select("CODGEO", "clust_ens")
classens$CODGEO <- as.character(classens$CODGEO)
disp_avignon <- left_join(disp_avignon, classens, by = "CODGEO")
...
comsf <- st_read(dsn = "C:/Users/clemm/Desktop/MASTER QESS 2ème année/Cours M2/Analyse spatiale/shapefile", stringsAsFactors = F)
...
mf_layout(title = "Classification des familles monoparentales dans l'unité urbaine d'Avignon (2018)",
 ...
 mf_theme("default")
```
```{r carto clust ensemble}
# run le début du code (partie carto comsf) pour que ça fonctionne
# carte
...
mf_label( topbot, 
          ...
          lines = TRUE,
          halo=FALSE)
mf_layout(title = "Classification de l'ensemble des familles dans l'unité urbaine d'Avignon (2018)",
    credits = "Données: INSEE, Filosofi, 2018",
    arrow = FALSE)
 mf_theme("default")
```
